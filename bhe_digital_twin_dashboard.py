...
with tab2:
    ...
    # Manual Plotly zoom-out would otherwise only enlarge the empty viewing
    # area while the forecast itself remains the same length. Locking the axes
    # keeps the graph fitted to the calculated domain.
    whatif_y_all = np.concatenate([
        baseline['Predicted'].to_numpy(dtype=float),
        changed['Predicted'].to_numpy(dtype=float),
    ])
    ...
    fig2.update_layout(
        title='Digital twin what-if forecast',
        xaxis_title='Elapsed time [h]',
